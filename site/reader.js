...
//
// This file is intentionally short. The rendering is Leaftext's own renderer,
// fetched as a module from leaftext.com (leaftext-core.js); the rest is
// minimap.js (the side-rail overview) and styles.css (what draws the page
// around the document).
// ---------------------------------------------------------------------------
...
// The renderer, loaded before anything is drawn. Every call below is behind
// main() having reached it.
let leaf = null;
...
// The raw TeX lives in each .math element's text (stashed by the renderer); we
 ...
 * whose landing page is TEI serves it here with nothing to add.
 * Returns { text, path }.
 */
  ...
  try {
    ...
    showStatus(
      'The reader could not be loaded (' +
        err.message +
        '), so this page cannot be drawn. It is served from leaftext.com, which may be down. The full text is in README.md beside this page.'
    );
    return;
  }
  try {
    ...
    // Auto-link glossary terms asynchronously after the page is displayed, checking for GLOSSARY.md or GLOSSARY.xml next to the document.
    installAutoGlossary({
      contentEl: content,
      render: renderDocument,
