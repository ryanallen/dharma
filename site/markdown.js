...
// GitHub references resolve against this repo (the site renders this one README).
const GH_REPO = 'ryanallen/leaftext';
const ghIssueAnchor = (repo, num, text) =>
  `<a class="github-ref issue-ref" href="https://github.com/${repo}/issues/${num}">${text}</a>`;
// Supported emoji shortcodes -> glyphs (a small, common set plus this doc's).
const EMOJI = {
  rocket: '🚀', tada: '🎉', warning: '⚠️', white_check_mark: '✅', shipit: '🚢',
  '+1': '👍', '-1': '👎', sparkles: '✨', fire: '🔥', bug: '🐛', memo: '📝',
  heavy_check_mark: '✔️', x: '❌', star: '⭐', zap: '⚡', leaves: '🍃',
};
...
export function renderMarkdown(src) {
  ...
  function renderInline(input) {
    ...
    s = s.replace(
      ...
      (_, alt, srcUrl, title) =>
        keep(
          `<img src="${escAttr(srcUrl)}" alt="${escAttr(alt)}" loading="lazy" decoding="async"` +
    ...
    s = s.replace(/:([a-z0-9_+-]+):/g, (m, code) =>
      EMOJI[code]
        ? keep(`<span class="emoji" aria-label=":${code}:" title=":${code}:">${EMOJI[code]}</span>`)
        : m
    );
    s = s.replace(/(^|[^\w/.-])([A-Za-z0-9][\w.-]*\/[A-Za-z0-9][\w.-]*)#(\d+)\b/g, (_m, pre, repo, num) =>
      pre + keep(ghIssueAnchor(repo, num, `${repo}#${num}`))
    );
    s = s.replace(/(^|[^\w&#/=-])#(\d+)\b/g, (_m, pre, num) =>
      pre + keep(ghIssueAnchor(GH_REPO, num, `#${num}`))
    );
    s = s.replace(/(^|[^\w/-])GH-(\d+)\b/g, (_m, pre, num) =>
      pre + keep(ghIssueAnchor(GH_REPO, num, `GH-${num}`))
    );
    s = s.replace(
  ...
  function blocks(lns) {
    ...
    while (i < n) {
      ...
      // Block math $$ … $$ — stash the raw TeX for KaTeX (reader.js).
      if (/^ {0,3}\$\$/.test(line)) {
        const body = [];
        const firstAfter = line.replace(/^ {0,3}\$\$/, '');
        if (/\$\$\s*$/.test(firstAfter) && firstAfter.replace(/\$\$\s*$/, '').trim() !== '') {
          body.push(firstAfter.replace(/\$\$\s*$/, ''));
          i++;
        } else {
          if (firstAfter.trim() !== '') body.push(firstAfter);
          i++;
          while (i < n && !/\$\$\s*$/.test(lns[i])) {
            body.push(lns[i]);
            i++;
          }
          if (i < n) {
            const last = lns[i].replace(/\$\$\s*$/, '');
            if (last.trim() !== '') body.push(last);
            i++;
          }
        }
        out.push('<div class="math math-block">' + esc(body.join('\n')) + '</div>');
        continue;
      }
